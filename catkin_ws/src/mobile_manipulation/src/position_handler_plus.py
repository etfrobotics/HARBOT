...
from mobile_manipulation.srv import MoveTask, MoveTaskRequest, PickPlaceTask, PickPlaceTaskRequest  # Replace 'your_package' with your actual package name
...
class PositionLoader:
    def __init__(self):
        ...
        # Subscribe to the topic where the commands are published
        self.command_sub = rospy.Subscriber('/task_stream', String, self.commands_callback)

        rospy.loginfo("PositionLoader initialized and waiting for commands")

    def commands_callback(self, msg):
        """Callback to process a list of commands received from the topic."""
        commands = msg.data.strip().split("\n")  # Assume commands are newline-separated
        rospy.loginfo(f"Received commands: {commands}")
        self.load_and_execute_tasks(commands)
